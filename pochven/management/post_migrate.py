...
def register_permissions(sender, **kwargs):
    ...
    fc_group.permissions.add(reset_scouts_perm)

    admin_permissions = Permission.objects.filter(
        Q(content_type__app_label__exact="auth", content_type__model__exact="user")
    )

    admin_group, _ = Group.objects.get_or_create(name="Admin")
    admin_group.permissions.add(reset_scouts_perm, *admin_permissions)
